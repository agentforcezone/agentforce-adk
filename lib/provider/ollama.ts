...
export class OllamaProvider implements OllamaProviderInterface {
    ...
    private getOllamaOptions(): { temperature: number; "num_ctx": number } {
        const defaultOptions = {
            temperature: 0.2, // Default temperature, can be overridden
            "num_ctx": 8192, // Default max tokens, can be overridden
        };
    ...
    async generateWithTools(prompt: string, tools: Tool[], system?: string, logger?: AgentForceLogger): Promise<string> {
        try {
            ...
            const fallback = await this.generate(prompt, system);
            return fallback;
        } catch (error) {
            throw new Error(`Ollama provider error: ${error}`);
        }
